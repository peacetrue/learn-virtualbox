guest.init.%: guest.create.% guest.vdi.load.% guest.nic.load.%;
# 创建客户机
guest.create.%:
#	创建客户机，主要是生成虚拟机相关的配置，--basefolder 虚拟机配置位置，--ostype 操作系统类型，--register 虚拟机中会显示客户机（反之不会）
	vboxmanage createvm --name $* --basefolder=$(BUILD) --ostype=Ubuntu22_LTS_64 --register
...
GUEST_INSTALLED=vboxmanage list vms | grep $*
GUEST_RUNNING=vboxmanage list runningvms | grep $* > /dev/null
# 启动客户机
guest.start.%:
	if ! $(GUEST_RUNNING); then  vboxmanage startvm $* --type=headless;  fi
...
$(BUILD)/%.guestproperty: guest.start.%
...
guest.ssh.%: $(BUILD)/%.ip
	ssh $(US_USER)@$(shell cat $<) || true

guest.shutdown.%:
	if $(GUEST_RUNNING); then  vboxmanage controlvm $* shutdown; sleep 2; echo "shutdown!";	fi
guest.clean.%: guest.shutdown.% clean/%.*
	vboxmanage unregistervm $* --delete

# 共享目录，%=客户机名称
HOSTPATH=$(workingDir)/learn-$*
guest.share.%: guest.shutdown.%
	vboxmanage sharedfolder add $* --name $* --hostpath $(HOSTPATH) --automount --auto-mount-point=/media/$*
